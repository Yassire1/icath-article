...
- The active pipeline forces `DEVICE = "cpu"` in `scripts/pipeline_config.py` even though `config/config.yaml` still says CUDA.
...
- Resume from a failed step: `python scripts/run_pipeline.py --from <step>`
- Focus a rerun: `python scripts/run_pipeline.py --only <steps...>`
- Fail fast on missing datasets during download: `python scripts/step_01_download.py --strict`
...
- The active runtime knobs are smoke-run sized in `scripts/pipeline_config.py`: `MAX_EVAL_SAMPLES=32`, `MAX_FEW_SHOT_SAMPLES=32`, `PATCHTST_MAX_TRAIN_WINDOWS=64`, `PATCHTST_MAX_STEPS=20`, `LORA_EPOCHS=2`, `TIMING_RUNS=1`.
